...
def parse_args(args_=None):
    ...
    # CBS parameters
    parser.add_argument('--cbs-kv-latency', type=float, default=5.0,
                        help='KV Cache transfer latency in ms (CBS)')
    parser.add_argument('--cbs-mu', type=float, default=2.0,
                        help='Risk penalty weight (CBS)')
    parser.add_argument('--cbs-lambda-ext', type=float, default=1.0,
                        help='Externality weight (CBS)')
    parser.add_argument('--cbs-kappa', type=float, default=0.1,
                        help='Dispatch contention coefficient (CBS)')
    parser.add_argument('--cbs-interference-table', type=str, default=None,
                        help='Path to interference table JSON (CBS)')
    parser.add_argument('--cbs-n-prefill', type=int, default=1,
                        help='Number of prefill instances (CBS)')
    parser.add_argument('--cbs-n-decode', type=int, default=1,
                        help='Number of decode instances (CBS)')
    parser.add_argument('--cbs-enable-migration', action='store_true', default=False,
                        help='Enable bidirectional migration (CBS)')
    parser.add_argument('--cbs-enable-role-adapt', action='store_true', default=False,
                        help='Enable role adaptation (CBS)')
    parser.add_argument('--cbs-slo-tpot', type=float, default=100.0,
                        help='TPOT SLO target in ms (CBS migration)')
    parser.add_argument('--cbs-slo-ttft', type=float, default=2000.0,
                        help='TTFT SLO target in ms (CBS role adaptation)')

    args = parser.parse_args(args=args_)

    assert args.backend in ['distserve', 'vllm', 'cbs'], f'Unknown backend: {args.backend}'
...
def main(args, outputs=None):
    ...
    if args.backend == 'vllm':
        ...
        cluster = CBSCluster(
            env=env,
            N_prefill_instance=args.cbs_n_prefill,
            N_decode_instance=args.cbs_n_decode,
            PP_prefill=PP_prefill, PP_decode=PP_decode,
            worker_configs=worker_config,
            kv_transfer_latency=args.cbs_kv_latency,
            mu=args.cbs_mu,
            lambda_ext=args.cbs_lambda_ext,
            kappa_dispatch=args.cbs_kappa,
            interference_table_path=args.cbs_interference_table,
            enable_migration=args.cbs_enable_migration,
            enable_role_adaptation=args.cbs_enable_role_adapt,
            slo_tpot=args.cbs_slo_tpot,
            slo_ttft=args.cbs_slo_ttft,
        )
    else:
